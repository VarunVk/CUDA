...
float computeOnDevice(float* h_data, int num_elements)
{
    ...
    float res=0.0f;
    float total=0.0f;
    int num_blocks = ceil(((float) num_elements/(2*BLOCK_SIZE)));
    cudaMalloc((void **)&data, sizeof(float)*num_elements);
    cudaMemcpy(data, h_data, sizeof(float)*num_elements, cudaMemcpyHostToDevice);


    dim3 grid = dim3(num_blocks,1,1);
    dim3 block = dim3(BLOCK_SIZE,1,1);

    reduction<<<grid,block>>>(data, num_elements);
    cudaDeviceSynchronize();

    for(int i=0;i<num_blocks;i++)
    {
        cudaMemcpy(&total, (const void *)(data+(i*2*BLOCK_SIZE)), sizeof(float), cudaMemcpyDeviceToHost);
        res += total;
        //printf("Res=%f total=%f h_data[%d] %f \n", res, total, i*2*BLOCK_SIZE, h_data[i*2*BLOCK_SIZE]);
    }
    return res;
}
