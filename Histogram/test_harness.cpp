...
int main(int argc, char* argv[])
{
    ...
    // Modify it to uint32_t so that while rolling over we can have some performance
    // improvement
    ...
    uint32_t *input_device = (uint32_t *)AllocateDeviceMemory(INPUT_WIDTH, INPUT_HEIGHT, sizeof(uint32_t));
    uint32_t *device_bins = (uint32_t *)AllocateDeviceMemory(HISTO_WIDTH,HISTO_HEIGHT, sizeof(uint32_t));

    cuda_memset(device_bins , 0, HISTO_WIDTH * HISTO_HEIGHT * sizeof(uint32_t));

    // copy the inputs from host to device
    CopyToDevice(input_device, &input[0][0], INPUT_HEIGHT, INPUT_WIDTH, sizeof(uint32_t));  
 
    /* End of setup code */

    /* This is the call you will use to time your parallel implementation */
    TIME_IT("opt_2dhisto", 50, opt_2dhisto(input_device, device_bins));
    //( /*Define your own function parameters*/ );)

    /* Include your teardown code below (temporary variables, function calls, etc.) */

    // copy the result from device to host
    CopyToHost(kernel_bins, device_bins, HISTO_WIDTH * HISTO_HEIGHT * sizeof(uint32_t));  

    // Free the device memory
    free_cuda(input_device);
    free_cuda(device_bins);
