...
//                h_data[i] = 1.0f;
    ...
    float* d_idata = NULL;
    float* d_odata = NULL;
    unsigned int numBlks_1 = ceil((float)num_elements/(2*BLOCK_SIZE));


    printf("%s %d num elements %d #blks %d #threads %d \n",__func__,__LINE__,
    			 num_elements, numBlks_1, BLOCK_SIZE);
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_idata, mem_size));
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_odata, mem_size));

    out1_H = (float*) malloc(num_elements*sizeof(float));
    in1_H  = (float*) malloc(numBlks_1*sizeof(float));
    ...
    CUDA_SAFE_CALL(cudaMemcpy( &h_data[1], d_odata, sizeof(float) * (num_elements-1), 
    ...
    //unsigned int result_regtest = cutComparef( reference, my_out_copy, num_elements);
