...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	Matrix A_d= AllocateDeviceMatrix(M);
	Matrix B_d= AllocateDeviceMatrix(N);
	Matrix C_d= AllocateDeviceMatrix(P);

	CopyToDeviceMatrix(A_d, M);
	CopyToDeviceMatrix(B_d, N);

#define TILE_LENGTH 2 
	dim3 grid = dim3(MATRIX_SIZE/TILE_LENGTH, MATRIX_SIZE/TILE_LENGTH, 1);
	dim3 block = dim3(TILE_LENGTH, TILE_LENGTH, 1);
	MatrixMulKernel<<<block, grid>>>(A_d, B_d, C_d);	
	
	cudaDeviceSynchronize();
	CopyFromDeviceMatrix(P, C_d);
